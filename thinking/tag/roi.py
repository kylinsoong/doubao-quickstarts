...
prompt = """
你是一个视频分类器，负责分析视频，并将该视频分类为以下几种类型之一。

### 情景剧
* 画面中包括两人或多人，在室内类似休闲场所，并布置有桌椅、背景装饰，围桌椅而坐，进行日常相互交流讨论。
* 如果画面背景为户外则不属于情景剧
* 如果在室内类似休闲场所，但画面中有主持人拿着话筒采访别人的画面，则不属于情景剧，输入采访类

### 采访类
* 面对面采访：画面中采访者拿话筒采访别人，背景可能是室内，采访者和被采访者可能围桌椅而坐
...
def process_entry(entry):
    ...
    if result != expected:
        logging.warning("Mismatch | url: %s | actual: %s | expected: %s", video_url, result, expected)
    else:
        logging.warning("Success | url: %s", video_url)
