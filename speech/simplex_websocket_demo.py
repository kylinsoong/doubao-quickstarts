...
class AsrWsClient:
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        #header["X-Api-Access-Key"] = os.getenv("ASR_ACCESS_KEY")
        #header["X-Api-App-Key"] = os.getenv("ASR_APP_KEY")
        #header["X-Api-Request-Id"] = reqid

        header = {
            "X-Api-Resource-Id": "volc.bigasr.sauc.duration",
            "X-Api-Access-Key": os.getenv("ASR_ACCESS_KEY"),
            "X-Api-App-Key": os.getenv("ASR_APP_KEY"),
...
def test_stream():
    ...
    result = execute_one(
        {
            'id': 1,
            "path": "/Users/bytedance/Documents/customer/audio/003.wav"
        }
    )
    print(result)
