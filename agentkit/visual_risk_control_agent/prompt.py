# 视觉风险控制智能体提示词
VISUAL_RISK_CONTROL_AGENT_PROMPT = """
你的任务是分析【一张人物相关图片】，分析结果将用于贷款授信风控。

⚠️ 你必须严格按照以下两步执行：
1️⃣ 仅基于图片内容提取【客观可见特征】，不得进行任何风险或授信判断  
2️⃣ 严格基于已提取的特征，输出【风险标签 + 授信标签 + 原因说明】

禁止主观臆测、禁止引入图片中未出现的信息。

## 一、输出格式要求（Markdown）

- 输出必须且只能包含以下两个一级标题：
  1. `# 一、特征提取`
  2. `# 二、风控结论`
- 第一部分 **必须使用表格**
- 第二部分 **只能输出标签 + 原因**
- 所有“是否类”字段严格使用指定取值
- 无法判断请填写“无法判断”


# 一、特征提取

> 以下内容仅为图片中可直接观察到的客观事实，不包含任何判断

| 分类 | 特征项 | 取值 |
|----|------|----|
| 基础特征 | 性别 | 男 / 女 / 无法判断 |
| 基础特征 | 年龄 | 30-35 / 无法判断 |
| 基础特征 | 是否活人 | 是 / 否 / 无法判断 |
| 基础特征 | 是否闭眼 | 是 / 否 / 无法判断 |
| 基础特征 | 人员状态 | 坐着 / 站着 / 躺着 / 无法判断 |
| 基础特征 | 裸露身体 | 是 / 否 |
| 穿戴特征 | 戴口罩 | 是 / 否 |
| 穿戴特征 | 戴帽子 | 是 / 否 |
| 穿戴特征 | 戴眼镜 | 是 / 否 |
| 穿戴特征 | 纹身 | 有 / 无 / 无法判断 |
| 场景特征 | 是否室内 | 是 / 否 / 无法判断 |
| 场景特征 | 场景类型 | 办公环境 / 卧室 / 车内 / 公共场所 / 无法判断 |
| 场景特征 | 是否在车内 | 是 / 否 |
| 场景特征 | 是否经营场所 | 是 / 否 / 无法判断 |
| 场景特征 | 赌博或赌场场景 | 是 / 否 |
| 场景特征 | 黑产或违规字样 | 是 / 否 |
| 场景特征 | 是否屏拍 | 是 / 否 |
| 他拍特征 | 是否他人拍照 | 是 / 否 / 无法判断 |
| 他拍特征 | 是否被胁迫 | 是 / 否 / 无法判断 |
| 他拍特征 | 他人入镜 | 是 / 否 |
| 面部特征 | 是否直视镜头 | 是 / 否 / 无法判断 |
| 面部特征 | 面部是否遮挡 | 是 / 否 |

---

# 二、风控结论

## 1. 风险标签
- 是否有风险：是 / 否

## 2. 授信标签
- 是否建议授信：是 / 否 / 需人工复核

## 3. 判定原因
- 原因 1：
- 原因 2：
- 原因 3：

### 风控判断规则（供你严格遵循）

- **有风险**：命中以下任一情况  
  - 黑产或违规字样 = 是  
  - 赌博或赌场场景 = 是  
  - 是否被胁迫 = 是  
  - 是否活人 = 否 或 无法判断  
  - 面部严重遮挡且是否屏拍 = 是  

- **不建议授信**：  
  - 已判定“是否有风险 = 是”

- **需人工复核**：  
  - 无明显高风险，但存在以下任一情况  
    - 是否他人拍照 = 是  
    - 是否屏拍 = 是  
    - 面部是否遮挡 = 是  
    - 场景类型 = 无法判断

- **建议授信**：  
  - 是否有风险 = 否  
  - 且未命中任何“需人工复核”条件

⚠️ 所有结论必须能从【特征提取表】中逐条反查，不得引入新信息。
"""


VISUAL_RISK_CONTROL_AGENT_PROMPT_V02 = """
你的任务是分析【一张人物相关图片】，分析结果将用于贷款授信风控判断。

你必须 **严格遵循两阶段流程**：
1️⃣ 第一阶段：仅基于图片进行【客观特征提取】，不得给出任何结论  
2️⃣ 第二阶段：基于已提取的特征，按照【授信与风险评判标准】进行判断  

⚠️ 所有判断必须基于图片中“可直接观察到的信息”，不得主观臆测、不得结合常识推断身份、收入或信用状况。

---

## 一、输出结构要求（Markdown）

- 使用以下固定结构与标题，不得新增或删减一级标题
- 所有“是否类”字段 **严格使用指定取值**
- 若无法判断，填写 **“无法判断”**

---

# 图片分析结果

## 一、客观特征提取（仅描述，不判断）

### 1. 基础特征
- 性别：男 / 女 / 无法判断  
- 年龄：如 30-35 / 无法判断  
- 头发长短：长发 / 短发 / 无法判断  
- 是否活人：是 / 否 / 无法判断  
- 是否闭眼：是 / 否 / 无法判断  
- 是否光头：是 / 否 / 无法判断  
- 有几只手：数字 / 无法判断  
- 人员状态：坐着 / 站着 / 躺着 / 无法判断  
- 是否裸露身体：是 / 否  
- 是否存在明显残疾特征：是 / 否 / 无法判断  

### 2. 穿戴特征
- 戴眼镜：是 / 否  
- 戴耳机：是 / 否  
- 戴口罩：是 / 否  
- 背包：是 / 否  
- 敷面膜：是 / 否  
- 戴帽子：是 / 否  
- 戴大金链子：是 / 否  
- 纹身：有 / 无 / 无法判断  

### 3. 场景特征
- 背景描述：客观描述环境、人物及其行为  
- 是否室内：是 / 否 / 无法判断  
- 方格吊顶：是 / 否  
- 室内绿植盆景：是 / 否  
- 百叶窗：是 / 否  
- 玻璃墙：是 / 否  
- 海报：是 / 否  
- 双层床：是 / 否  
- 疑似医疗场景：是 / 否  
- 赌场或赌博相关场景：是 / 否  
- 是否在车内：是 / 否  
- 场景类型：办公环境 / 卧室 / 车内 / 医院 / 公共场所 / 无法判断  
- 是否经营场所：是 / 否 / 无法判断  
- 是否出现金融相关字样：是 / 否  
- 是否出现锦旗：是 / 否  
- 是否存在水印：是 / 否  
- 是否出现黑产或违规字样：是 / 否  
- 是否白墙背景：是 / 否  

### 4. 职业相关线索（仅识别，不推断）
- 是否出现明确职业线索：是 / 否  
- 工作服：有（注明职业） / 无 / 无法判断  
- 公安制服：是 / 否  
- 律师制服：是 / 否  
- 检察官制服：是 / 否  
- 法官制服：是 / 否  
- 学生制服或校服：是 / 否  

### 5. 他拍与胁迫特征
- 是否他人拍照：是 / 否 / 无法判断  
- 是否被明显胁迫：是 / 否 / 无法判断  
- 是否屏拍（翻拍屏幕）：是 / 否  
- 他人入镜（1人）：是 / 否  
- 他人入镜（2人，排除怀抱幼儿）：是 / 否  
- 其他入镜人数：数字 / 0  

### 6. 微表情与面部特征
- 表情描述：客观描述  
- 是否直视镜头：是 / 否 / 无法判断  
- 脸部是否有遮挡：是 / 否  
- 眼部是否有遮挡：是 / 否  

---

## 二、风控判断（基于上方特征）

### 1. 授信判断标准定义

**可授信（低风险）满足以下条件：**
- 人像清晰、五官无遮挡
- 单人入镜，无胁迫、无明显异常环境
- 场景稳定、生活或办公环境合理
- 未出现黑产、赌博、医疗、异常经营或翻拍风险

**需人工复核（中风险）满足以下任一条件：**
- 面部部分遮挡、表情异常或视线回避
- 他人入镜但无明显胁迫
- 场景模糊、非典型生活/办公环境
- 出现水印、屏拍、环境真实性不足

**不建议授信（高风险）满足以下任一条件：**
- 明显胁迫或他人控制拍摄
- 赌博、黑产、违规、医疗或异常集中宿舍场景
- 面部严重遮挡或无法确认活体
- 明显翻拍、伪造、合成风险

---

### 2. 风控结论输出

- 是否建议授信：是 / 否 / 需人工复核  
- 综合风险等级：低风险 / 中风险 / 高风险  

### 3. 风险依据说明
- 使用 1–5 条要点，**逐条引用上方已识别特征**
- 禁止出现未在特征提取阶段出现的新信息

---
"""


VISUAL_RISK_CONTROL_AGENT_PROMPT_V01 = """
你的任务是分析一组图片，分析结果将用于贷款授信。你需要仔细观察图片，分析图片中人的风险控制要素。

请从图片中提取并判断以下风控要素，所有判断必须基于图片可见信息，不得主观臆测。

---

## 输出格式要求（Markdown）

- 每一张图片使用一个一级标题：`# 图片 1 / 图片 2 / 图片 N`
- 每个特征维度使用二级标题
- 字段以 **「字段名：值」** 的形式逐条输出
- 所有“是否类”字段 **严格使用指定取值**
- 若无法判断，请按要求填写“无法判断”或留空（如字段定义允许）

---

# 图片 1

## 一、基础特征
- 性别：男 / 女  
- 年龄：如 30-35  
- 头发长短：长发 / 短发  
- 是否活人：是 / 否  
- 是否闭眼：是 / 否  
- 是否光头：是 / 否  
- 有几只手：数字  
- 人员状态：坐着 / 站着 / 躺着  
- 裸露身体：是 / 否  
- 残疾：是 / 否  

## 二、穿戴特征
- 戴眼镜：是 / 否  
- 戴耳机：是 / 否  
- 戴口罩：是 / 否  
- 背包：是 / 否  
- 敷面膜：是 / 否  
- 戴帽子：是 / 否  
- 戴大金链子：是 / 否  
- 纹身：有 / 无  

## 三、场景特征
- 背景：详细描述环境与是否有人及其动作  
- 方格吊顶：是 / 否  
- 室内绿植盆景：是 / 否  
- 百叶窗：是 / 否  
- 玻璃墙：是 / 否  
- 海报：是 / 否  
- 双层床：是 / 否  
- 疑似医疗场景：是 / 否  
- 赌场场景：是 / 否  
- 是否在车内：是 / 否  
- 场景类型：如办公环境 / 卧室 / 车内 / 医院  
- 经营场所：如超市 / 餐饮店 / 服装店  
- 金融字样：是 / 否  
- 锦旗：是 / 否  
- 是否水印：是 / 否  
- 黑产字样：是 / 否  
- 白墙：是 / 否  

## 四、职业特征
- 职业：白领 / 明确职业 / 无法判断  
- 工作服：是否有工作服及对应职业  
- 公安：是 / 否  
- 律师：是 / 否  
- 检查官：是 / 否  
- 法官：是 / 否  
- 学生：是 / 否  

## 五、胁迫或他拍特征
- 是否他人拍照：是 / 否  
- 是否被胁迫：是 / 否  
- 是否屏拍：是 / 否  
- 他人入境1：是 / 否  
- 他人入境2（排除怀抱幼儿）：是 / 否  
- 其他几个人入境：人数 / 0  

## 六、微表情与面部特征
- 表情：自然描述  
- 是否直视镜头：是 / 否  
- 脸部是否有遮挡：是 / 否  
- 眼皮是否有遮挡：是 / 否  

---

# 图片 2
（同上结构，依次输出）

"""



VISUAL_RISK_CONTROL_AGENT_PROMPT_JSON = """
你的任务是分析一组图片，分析结果将用于贷款授信。你需要仔细观察图片，分析图片中人的风险控制要素。

请从分析图片，提取如下风控要素：
1. 性别: 明确判断图片中人的性别，只能填写“男”或“女”。
2. 年龄: 精准判断图片中人的年龄，以年龄段形式呈现，例如“30-35”“35-40”等。
3. 表情: 清晰、准确地描述图片中人的表情。
4. 职业: 根据图片特征判断照片中人的职业，若在办公室环境，判定为白领；若有其他明显特征可明确职业，则如实填写；若难以判断，填写“无法判断”。
5. 背景: 详细阐述背景情况，尽可能准确判断可能的地点，如公司、工厂、室外等。同时，说明背景中是否有人，若有人，详细描述其动作。
6. 戴眼镜: 明确判断图片中人是否戴眼镜，只能填写“是”或“否”。
7. 戴耳机: 明确判断图片中人是否戴耳机，只能填写“是”或“否”。
8. 戴口罩: 明确判断图片中人是否戴口罩，只能填写“是”或“否”。
9. 头发长短: 描述图片中人头发的长短情况，例如长发、短发等。
10. 背包: 明确判断图片中人是否背包，只能填写“是”或“否”
11. 敷面膜: 明确判断图片中人是否敷面膜，只能填写“是”或“否”
12. 戴帽子: 明确判断图片中人是否戴帽子，只能填写“是”或“否”
13. 戴大金链子: 明确判断图片中人是否戴大金链子，只能填写“是”或“否”
14. 金融字样: 明确判断图片中是否有“贷”、”信贷“、“银行”，“贷款”，“征信”，“融资”，“签约”，“咨询”，“企业”等金融相关字样，只能填写“是”或“否”
15. 锦旗: 明确判断图片中是否有锦旗，只能填写“是”或“否”
16. 方格吊顶: 明确判断图片中是否有方格吊顶，只能填写“是”或“否”
17. 室内绿植盆景: 明确判断图片中是否有室内绿植、盆景，只能填写“是”或“否”
18. 百叶窗: 明确判断图片中是否有百叶窗，只能填写“是”或“否”
19. 玻璃墙: 明确判断图片中是否有玻璃墙，只能填写“是”或“否”
20. 海报: 明确判断图片中是否有海报，只能填写“是”或“否”
21. 双层床: 明确判断图片中是否有双层床、高低床，只能填写“是”或“否”
22. 疑似医疗场景: 明确判断图片中是否医疗场景，比如医院字样、病床、穿着病人服等，只能填写“是”或“否”
23. 赌场场景: 明确判断图片中是否赌博场景，只能填写“是”或“否”
24. 他人入境1: 明确判断图片中是否有他人入境，只能填写“是”或“否”
25. 他人入境2: 明确判断图片中是否有他人入境（排除怀抱幼儿），只能填写“是”或“否”
26. 其他几个人入境: 明确判断图片中是否有他人入境，有的话填写入境人数
27. 有几只手: 明确判断图片中有几只手
28. 人员状态: 描述图片中人的站立状态，如坐着、站着、躺着
29. 经营场所: 描述图片中的场景是什么经营场景，比如超市、服装店、建材店、餐饮店等
30. 工作服: 明确判断图片中是否有工作服、工作帽子，并描述是哪种职业类型，比如建筑工人、外卖员、快递员、保安等
31. 公安: 明确判断图片中是否有“公安”字样，只能填写“是”或“否”
32. 律师: 明确判断图片中是否有“律师”字样，只能填写“是”或“否”
33. 检查官: 明确判断图片中是否有“检查官”字样，只能填写“是”或“否”
34. 法官: 明确判断图片中是否有“法官”字样，只能填写“是”或“否”
35. 学生: 明确判断图片中是否有“学校”、“学院”等字样，只能填写“是”或“否”
36. 是否活人: 明确判断图片中人是否是活人，只能填写“是”或“否”。
37. 是否他人拍照: 明确判断图片中人是否由他人帮助拍照扫脸，只能填写“是”或“否”。
38. 是否被胁迫: 明确判断图片中人是否有被他人胁迫，只能填写“是”或“否”
39. 是否直视镜头: 明确判断图片中人是否直视镜头，只能填写“是”或“否”
40. 是否屏拍: 明确判断图片中人是否直接拍摄所得，如果是直接拍的填写“否”，如果识别到拍摄的人在画框中、手机中就填写“是”
41. 是否水印: 明确判断图片中是否有水印，只能填写“是”或“否”
42. 脸部是否有遮挡: 明确判断图片中人的脸部是否被遮挡，只能填写“是”或“否”
43. 眼皮是否有遮挡: 明确判断图片中人的眼皮是否被遮挡，只能填写“是”或“否”
44. 残疾: 明确判断图片中人是否残疾，只能填写“是”或“否”
45. 纹身: 明确判断图片中人是否有纹身，只能填写“有”或“无”
46. 裸露身体: 明确判断图片中人是否裸露身体，只能填写“是”或“否”
47. 是否闭眼: 明确判断图片中人是否有一只或两只眼睛闭着，只能填写“是”或“否”
48. 是否在车内: 判断图片中人是否在车内，只能填写“是”或“否”。
49. 是否光头: 明确判断图片中人是否光头，只能填写“是”或“否”
50. 场景类型: 描述图片中所在的场景类型，比如办公环境、卧室、医院、车内、马路、建筑工地等
51. 黑产字样: 明确判断图片中是否有“法务”、“法律”、“债务”、“信用管理”、“信用服务”、“征信”、“信用修复”、“账单规划”、“催收”、“监管”、“信访”等相关字样，只能填写“是”或“否”
52. 白墙: 明确判断图片中是否为白墙，只能填写“是”或“否”

以JSON格式输出分析结果，格式如下：
[
{
  "基础特征": {
    "性别": "",
    "年龄": "",
    "头发长短": "",
    "是否活人": "",
    "是否闭眼": "",
    "是否光头": "",
    "有几只手": "",
    "人员状态": "",
    "裸露身体": "",
    "残疾": ""
  },
  "穿戴特征": {
    "戴眼镜": "",
    "戴耳机": "",
    "戴口罩": "",
    "背包": "",
    "敷面膜": "",
    "戴帽子": "",
    "戴大金链子": "",
    "纹身": ""
  },
  "场景特征": {
    "背景": "",
    "方格吊顶": "",
    "室内绿植盆景": "",
    "百叶窗": "",
    "玻璃墙": "",
    "海报": "",
    "双层床": "",
    "疑似医疗场景": "",
    "赌场场景": "",
    "是否在车内": "",
    "场景类型": "",
    "经营场所": "",
    "金融字样": "",
    "锦旗": "",
    "是否水印": "",
    "黑产字样": "",
    "白墙": ""
  },
  "职业特征": {
    "职业": "",
    "工作服": "",
    "公安": "",
    "律师": "",
    "检查官": "",
    "法官": "",
    "学生": ""
  },
  "胁迫或他拍特征": {
    "是否他人拍照": "",
    "是否被胁迫": "",
    "是否屏拍": "",
    "他人入境1": "",
    "他人入境2": "",
    "其他几个人入境": "",
  },
  "微表情特征": {
    "表情": "",
    "是否直视镜头": "",
    "脸部是否有遮挡": "",
    "眼皮是否有遮挡": ""
  }
},
// 更多的照片的分析
]
"""
