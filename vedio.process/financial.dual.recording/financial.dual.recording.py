...
def generate_prompt(asr_results):
    original_prompt = load_file_content("prompt.financial.dual.recording.ini")
    prompt = original_prompt.replace("{{ASR_JSON_SUBTITLES}}", asr_results)
    return prompt

def process_videos(asr, vlm, obj_keys):
...
@log_time
def main():
    asr, vlm = initialize_clients()

    obj_keys = ["3600/101.mp4", "3600/101.mp4"]

    process_videos(asr, vlm, obj_keys)
