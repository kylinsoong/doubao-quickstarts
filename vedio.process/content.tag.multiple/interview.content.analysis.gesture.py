...
def generate_prompt(asr_results):
    original_prompt = load_file_content("prompt.content.tag.multiple.gesture.ini")
    prompt = original_prompt.replace("{{ASR_JSON_SUBTITLES}}", asr_results)
    return prompt

def process_videos(tos, asr, vlm, urls):
    for url in urls:
...
def main():
    ...
    obj_keys = ["https://pub-kylin.tos-cn-beijing.volces.com/0001/201.mov"]
